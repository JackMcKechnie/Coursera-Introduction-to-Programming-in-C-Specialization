...
int card_ptr_comp(const void * vp1, const void * vp2) {
  ...
  //If cards are tied on value use enum value of suit to compare
  else{
      return 0;
  }
}
...
//int compare_hands(deck_t * hand1, deck_t * hand2) {
  //Sort the hands using quicksort
//  qsort(hand1->cards,hand1->n_cards,sizeof(hand1->cards[0]),card_ptr_comp);
//  qsort(hand2->cards,hand2->n_cards,sizeof(hand2->cards[0]),card_ptr_comp);

  //Get a hand_eval_t for both hands
...
int get_match_count(unsigned * values, size_t nvalues,int ifrom){
  //Helper function for get_match_counts
  int i = ifrom + 1;
  int count = 1;
...
unsigned * get_match_counts(deck_t * hand) {
  ...
  //Allocate an array that is the same size as the hand
  unsigned * counts = malloc(hand->n_cards * sizeof(*counts));

  //Temp array
  unsigned * values = malloc(hand->n_cards * sizeof(*counts));
  //Loop through the cards in the hand and add them to value
  for(int i=0;i<hand->n_cards;i++){
